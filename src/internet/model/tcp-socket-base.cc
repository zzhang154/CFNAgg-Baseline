...
TcpSocketBase::Recv(uint32_t maxSize, uint32_t flags)
{
    ...
    {
        if(m_tcb->m_rxBuffer->Size() == 0)
            NS_LOG_DEBUG("m_tcb->m_rxBuffer->Size() == 0");
        if(m_state == CLOSE_WAIT)
            NS_LOG_DEBUG("m_state == CLOSE_WAIT");
...
TcpSocketBase::SendPendingData(bool withAck)
{
    ...
    while (availableWindow > 0)
    {
        ...
        else
        {
            ...
            NS_LOG_DEBUG("availableData: " << availableData << " availableWindow: " << availableWindow
                                           << " next: " << next << " nextHigh: " << nextHigh
                                           << " m_tcb->m_segmentSize: " << m_tcb->m_segmentSize);
...
TcpSocketBase::ReceivedData(Ptr<Packet> p, const TcpHeader& tcpHeader)
{
    ...
    */
    ...
    NS_LOG_DEBUG("Expected sequence number: " << expectedSeq);
