...
QuicSocketBase::GetTypeId (void)
{
  static TypeId tid = TypeId ("ns3::QuicSocketBase")
    ...
    .AddAttribute ("MaxPacketSize", "Maximum Packet Size",
                   UintegerValue (UINT16_MAX-100),//1460  1620  //1840 //Zhuoxu: try 25000 here
...
QuicSocketBase::Send (Ptr<Packet> p, uint32_t flags)
{
  ...
  int data = 0;
  //std::cout<<"QuicSocketBase::Send (Ptr<Packet> p, uint32_t flags)"<<p<<std::endl;
...
{//std::cout<<"QuicSocketBase::Send (Ptr<Packet> p)"<<p<<std::endl;
  NS_LOG_FUNCTION (this);
  //std::cout<<"m_drainingPeriodEvent.IsRunning ():----"<<m_drainingPeriodEvent.IsRunning ()<<std::endl;
  if (m_drainingPeriodEvent.IsRunning ())
    {
      NS_LOG_INFO ("Socket in draining state, cannot send packets");
      return 0;
...
QuicSocketBase::DoRetransmit (std::vector<Ptr<QuicSocketTxItem> > lostPackets)
{
  ...
  //std::cout<<toRetx << " bytes to retransmit"<<"Send the retransmitted frame-on--conID-"<<m_connectionId<<"--on time--"<<Simulator::Now()<<std::endl;
  uint32_t win = AvailableWindow ();
  uint32_t connWin = ConnectionWindow ();
  uint32_t bytesInFlight = BytesInFlight ();
  NS_LOG_DEBUG (
  ...
  /*std::cout<<"BEFORE Available Window " << win
                               << " Connection RWnd " << connWin
                               << " BytesInFlight " << bytesInFlight
                               << " BufferedSize " << m_txBuffer->AppSize ()
                               << " MaxPacketSize " << GetSegSize ()
...
QuicSocketBase::ReTxTimeout ()
{
  ...
  //std::cout<<"ReTxTimeout Expired at time " << Simulator::Now ().GetSeconds ()<<std::endl;
  // Handshake packets are outstanding)
  if (m_tcb->m_alarmType == 0 && (m_socketState == CONNECTING_CLT || m_socketState == CONNECTING_SVR))
  ...
  else if (m_tcb->m_alarmType == 1 && m_tcb->m_lossTime != Seconds (0))
    {
      ...
      //std::cout<<"RTO triggered: early retransmit"<<std::endl;
      ...
      //std::cout<<"ReTxTimeout-----doretransmit---"<<lostPackets.data()<<std::endl;
...
QuicSocketBase::SendTo (Ptr<Packet> p, uint32_t flags, const Address &address)
{
  ...
  //std::cout<<"QuicSocketBase::SendTo (Ptr<Packet> p"<<p<<std::endl;
  return Send (p, flags);
}
...
{
  //std::cout<<"quic-socket-base-receving -----"<<maxSize<<"size of packets"<<std::endl;
  NS_LOG_FUNCTION (this);
...
{
  NS_LOG_FUNCTION (this<<m_txBuffer->Available ());
  ...
  if (!lostPackets.empty ())
    {
      ...
      std::cout<<"Onreceivedackframe----DoRetransmit (lostPackets);"<<std::endl;
      DoRetransmit (lostPackets);
    }
...
Ptr<QuicSocketState>
QuicSocketBase::GetSocketStatetcb () 
{
  return m_tcb;
}
