                                  ...
                                  const struct RateSample *rs)
{
  ...
  std::cout<<"QuicCongestionOps::OnAckReceived---"<<std::endl;
  Ptr<QuicSocketState> tcbd = dynamic_cast<QuicSocketState*> (&(*tcb));
  NS_ASSERT_MSG (tcbd != nullptr, "tcb is not a QuicSocketState");
...
std::cout<<"QuicCongestionOps::OnAckReceived-11111111111--"<<std::endl;
...
std::cout<<"QuicCongestionOps::OnAckReceived-222222--"<<std::endl;
...
//          }
  ...
  std::cout<<"Update smoothed RTT"<<std::endl;
  // Based on [RFC6298].
  ...
  std::cout<<"Update smoothed RTTif (tcbd->m_smoothedRtt == Seconds (0))"<<tcbd->m_smoothedRtt.GetSeconds()<<std::endl;
    }
  else
    {
  ...
  std::cout<<"Update smoothed RTT"<<tcbd->m_smoothedRtt.GetSeconds()<<std::endl;
    }
